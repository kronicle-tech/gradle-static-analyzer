...
public abstract class BaseVisitor extends CodeVisitorSupport {
    ...
    public BaseVisitor(BaseVisitorDependencies dependencies) {
        buildFileLoader = dependencies.getBuildFileLoader();
        buildFileProcessor = dependencies.getBuildFileProcessor();
        expressionEvaluator = dependencies.getExpressionEvaluator();
        softwareRepositoryFactory = dependencies.getSoftwareRepositoryFactory();
    }
    ...
    protected void addSoftwareRepository(String url) {
        getSoftwareRepositories().add(softwareRepositoryFactory.createSoftwareRepository(url, getSoftwareRepositoryScope()));
    }
            ...
            if (call.getArguments() instanceof TupleExpression) {
                ...
                if (arguments.getExpressions().size() == 1 && arguments.getExpression(0) instanceof NamedArgumentListExpression) {
                    ...
                    if (values.size() == 1 && values.containsKey("plugin")) {
                        ...
                        throw new RuntimeException(String.format("apply call with unexpected arguments %s", String.join(", ", values.keySet())));
                    }
                }
            }
        }
